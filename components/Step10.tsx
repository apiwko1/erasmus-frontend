...
const Step8: React.FC<Props> = ({ exam }) => {
    ...
    const [query1, setQuery1] = useState(Number(exam?.time1 || '0'));
    const [query1Error, setQuery1Error] = useState(false);

    const [query2, setQuery2] = useState(Number(exam?.time2 || '0'));
    const [query2Error, setQuery2Error] = useState(false);

    const [query3, setQuery3] = useState(Number(exam?.time3 || '0'));
    const [query3Error, setQuery3Error] = useState(false);

    const [query4, setQuery4] = useState(Number(exam?.time4 || '0'));
    ...
    return (
        <div>
            ...
            <div className="formContainer">
                ...
                <div className="field">
                    <div className="fieldLabel">{t('time1')}</div>
                    <RatingField
                        name="hover-time1"
                        value={query1}
                ...
                <div className="field">
                    <div className="fieldLabel">{t('time2')}</div>
                    <RatingField
                        name="hover-time2"
                        value={query2}
                        onChange={(event, newValue) => {
                ...
                <div className="field">
                    <div className="fieldLabel">{t('time3')}</div>
                    <RatingField
                        name="hover-time3"
                        value={query3}
                        onChange={(event, newValue) => {
                ...
                <div className="field">
                    <div className="fieldLabel">{t('time4')}</div>
                    <RatingField
                        name="hover-time4"
                        value={query4}
                        onChange={(event, newValue) => {
            ...
            <div className="button" >
                <Button variant="contained" color="primary" onClick={() => router.push(`/${locale}/${token}/step9`)} style={{ marginRight: 10 }}>{t('back')}</Button>
